function[OpUnit, OpFinal, Res_Comb] = Dist_Unit_Opt_conc(Comb, Init_Env, N, m, dim, exitTolerance)

%function that returns the best sequence of decoupling operations for m
%concatenations of a given comb with N slots. Optimization via minimization
%of distance to the unitaries
...
for i = 1:m
    ...
    %round
    [Env_new, DD_Seq] = Dist_Unit_One_It(Comb, N, dim, Env_new, exitTolerance);
...
[Env_new, DD_Seq] = Dist_Unit_One_It(Res_Comb, N, dim, Init_Env, exitTolerance);
